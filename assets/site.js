/* Shared site scripts: nav toggle, scroll-reveal, coverflow, join form. All guarded. */
  ...
  (function () {
    ...
    function blogCard(b, i) {
      var k = i % 3;
      var href = b.id ? ('post.html?id=' + encodeURIComponent(b.id)) : '#';
      var media = b.image
        ? '<a href="' + href + '" class="block relative h-44 overflow-hidden"><img src="' + esc(b.image) + '" alt="' + esc(b.title) + '" loading="lazy" class="w-full h-full object-cover transition-transform duration-500 hover:scale-105"><span class="absolute top-4 left-4 bg-black/50 backdrop-blur text-white text-[11px] font-semibold tracking-wide px-3 py-1 rounded-full border border-white/20">' + esc(b.category) + '</span></a>'
        : '<a href="' + href + '" class="block relative h-44" style="background: ' + BG[k] + ';"><span class="absolute top-4 left-4 bg-black/40 backdrop-blur text-white text-[11px] font-semibold tracking-wide px-3 py-1 rounded-full border border-white/20">' + esc(b.category) + '</span>' + ICON[k] + '</a>';
      return '<div class="h-full"><article class="glass-card rounded-2xl overflow-hidden h-full flex flex-col hover:-translate-y-1.5 transition-transform duration-300">'
        ...
        + '<h3 class="font-serif text-lg font-bold text-white mb-2 line-clamp-2"><a href="' + href + '" class="hover:text-brand-gold transition-colors">' + esc(b.title) + '</a></h3><p class="text-sm text-gray-400 leading-relaxed line-clamp-2 mb-5 flex-1">' + esc(b.excerpt) + '</p>'
        + '<a class="inline-flex items-center gap-1 text-xs font-bold text-brand-gold hover:gap-2 transition-all mt-auto" href="' + href + '">ಓದಿರಿ ' + ARROW + '</a></div></article></div>';
    }
    ...
    function mapWp(type, it) {
      ...
      if (type === 'blog') {
        ...
        var img = ''; try { img = it._embedded['wp:featuredmedia'][0].source_url; } catch (e) {}
        return { id: it.id, category: cat || f('category') || '', date: (it.date || '').slice(0, 10), readtime: f('readtime') || '', title: t, excerpt: strip(it.excerpt && it.excerpt.rendered), image: img || '' };
      }
